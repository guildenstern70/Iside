...
namespace LLCryptoLib.Security
{
    ...
    internal struct DataBlob
    {
        ...
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Reliability",
             "CA2006:UseSafeHandleToEncapsulateNativeResources")] public IntPtr pbData;
    }
    ...
    /// <summary>
    ///     The TrustListUsage structure contains an array of Object Identifiers (OIDs) for Certificate Trust List (CTL)
    ///     extensions. CTL_USAGE structures are used in functions that search for CTLs for specific uses.
    /// </summary>
    [StructLayout(LayoutKind.Sequential, CharSet = CharSet.Ansi)]
    ...
    /// <summary>
    ///     The CertificateExtension structure contains the extension information for a certificate, Certificate Revocation
    ...
    /// <summary>
    ///     The CertificateNameValue structure contains a relative distinguished name (RDN) attribute value. It is like the
    ///     CERT_RDN_ATTR structure, except that it does not include the object identifier member that is a member of
    ///     CERT_RDN_ATTR. As in CERT_RDN_ATTR, the interpretation of the Value member depends on dwValueType.
    /// </summary>
    [StructLayout(LayoutKind.Sequential, CharSet = CharSet.Ansi)]
    ...
    /// <summary>
    ///     The CertificateNameInfo structure contains subject or issuer names. The information is represented as an array of
    ///     CERT_RDN structures.
    /// </summary>
    [StructLayout(LayoutKind.Sequential, CharSet = CharSet.Ansi)]
    ...
    /// <summary>
    ///     The RelativeDistinguishedName structure contains a relative distinguished name (RDN) consisting of an array of
    ///     CERT_RDN_ATTR structures.
    /// </summary>
    [StructLayout(LayoutKind.Sequential, CharSet = CharSet.Ansi)]
    ...
    /// <summary>
    ///     The RdnAttribute structure contains a single attribute of a relative distinguished name (RDN). A whole RDN is
    ///     expressed in a CERT_RDN structure that contains an array of CERT_RDN_ATTR structures.
    /// </summary>
    [StructLayout(LayoutKind.Sequential, CharSet = CharSet.Ansi)]
    ...
    /// <summary>
    ///     The ChainParameters structure establishing the searching and matching criteria to be used in building a certificate
    ///     chain.
    /// </summary>
    [StructLayout(LayoutKind.Sequential, CharSet = CharSet.Ansi)]
    ...
    /// <summary>
    ///     The ChainPolicyStatus structure holds certificate chain status information returned by
    ///     CertVerifyCertificateChainPolicy from the verification of certificate chains.
    /// </summary>
    [StructLayout(LayoutKind.Sequential, CharSet = CharSet.Ansi)]
    ...
    /// <summary>
    ///     The ChainPolicyParameters structure contains information used in CertVerifyCertificateChainPolicy to establish
    ///     policy criteria for the verification of certificate chains.
    /// </summary>
    [StructLayout(LayoutKind.Sequential, CharSet = CharSet.Ansi)]
    ...
    /// <summary>
    ///     The CRYPT_KEY_PROV_INFO structure contains fields that are passed as the arguments to CryptAcquireContext to
    ///     acquire a handle to a particular key container within a particular cryptographic service provider (CSP), or to
    ///     create or destroy a key container.
    /// </summary>
