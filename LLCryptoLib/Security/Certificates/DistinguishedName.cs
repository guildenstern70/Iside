...
namespace LLCryptoLib.Security.Certificates
{
    ...
    public class DistinguishedName
    {
        ...
        {
            int size = 0;
            SspiProvider.CryptDecodeObject(SecurityConstants.PKCS_7_ASN_ENCODING | SecurityConstants.X509_ASN_ENCODING,
                new IntPtr(SecurityConstants.X509_UNICODE_NAME), input, length, 0, IntPtr.Zero, ref size);
            if (size <= 0)
            ...
            if (
                SspiProvider.CryptDecodeObject(
                    ...
                    new IntPtr(SecurityConstants.X509_UNICODE_NAME), input, length, 0, buffer, ref size) == 0)
                throw new CertificateException("Unable to decode the name of the certificate.");
            try
            {
                CertificateNameInfo cni =
                    (CertificateNameInfo) Marshal.PtrToStructure(buffer, typeof(CertificateNameInfo));
                this.Initialize(cni);
            }
        ...
        {
            get { return (NameAttribute) this.m_List[index]; }
        ...
        /// <summary>
        ///     Initializes a new instance of the <see cref="DistinguishedName" /> class with a given
        ///     <see cref="CertificateNameInfo" /> instance.
        /// </summary>
            ...
            for (int i = 0; i < cni.cRDN; i++)
            {
                cr =
                    ...
                    Marshal.PtrToStructure(
                        new IntPtr(cni.rgRDN.ToInt64() + i*Marshal.SizeOf(typeof(RelativeDistinguishedName))),
                        typeof(RelativeDistinguishedName));
                for (int j = 0; j < cr.cRDNAttr; j++)
                {
                    cra =
                        (RdnAttribute)
                        Marshal.PtrToStructure(
                            new IntPtr(cr.rgRDNAttr.ToInt64() + j*Marshal.SizeOf(typeof(RdnAttribute))),
                            typeof(RdnAttribute));
                    this.m_List.Add(new NameAttribute(Marshal.PtrToStringAnsi(cra.pszObjId),
                        Marshal.PtrToStringUni(cra.pbData)));
                }
            }
        }
        ...
        /// <summary>
        ///     Searches for an Object with the specified Object identifier and returns the zero-based index of the first
        ...
        public int IndexOf(string oid)
        {
            ...
            {
                if (((NameAttribute) this.m_List[i]).ObjectID == oid)
                    return i;
            }
            return -1;
        }
