...
namespace LLCryptoLib.Shred
{
    ...
    public class Shredder
    {
        ...
        {
            this.rnd = rand;
            this.LastShreddedPath = string.Empty;
        }
        ...
        public string LastShreddedPath { get; private set; }
        ...
        /// <param name="method">Shredding method</param>
        /// <param name="cbe">
        ///     Callback entry point for feedback. The callback method will be called every 1024 bytes shredded. An
        ///     integer will be passed to callback method with the current kilobyte being erased. For instance, if you want to
        ///     erase a 200kb file, this delegate will call the method for 200 times passing to it integers from 1 to 200.
        /// </param>
        ...
        ///     True if the file has been successfully shredded
        /// </returns>
        public bool WipeFile(FileInfo filePath, IShredMethod method, CallbackEntry cbe, CallbackPoint phase,
            AutoResetEvent resEvent, bool delete)
        {
            if (filePath == null)
            {
            ...
            bool status = WipeFile(filePath.FullName, method, cbe, phase, resEvent);
            ...
            if (status)
            {
                ...
                try
                {
                    ...
                    this.LastShreddedPath = Path.Combine(filePath.DirectoryName, newname);

                    if (delete)
                    {
                        System.Diagnostics.Debug.WriteLine("Deleting " + this.LastShreddedPath);
                        File.Delete(this.LastShreddedPath);
                    }
                }
                catch (Exception e)
                {
        ...
        /// <returns>
        ///     A random filename of 'characters' number of character. A dot will be placed before the last three characters,
        ///     ie: ejeskjekjw.ejh
        /// </returns>
            ...
            if (di.Exists)
            {
                if (di.GetDirectories().Length == 0)
                {
                    if (di.GetFiles().Length == 0)
                    {
                        try
                        {
                            ...
                            this.LastShreddedPath = Path.Combine(di.Parent.FullName, newfoldername);
                            if (!Directory.Exists(this.LastShreddedPath))
                            {
                                di.MoveTo(this.LastShreddedPath);
                                newDirectoryName = newfoldername;
                                System.Diagnostics.Debug.WriteLine("Renamed directory: " + this.LastShreddedPath);
                            }
                            else
                            {
                            ...
                            DirectoryInfo toDelete = new DirectoryInfo(this.LastShreddedPath);
                            if (toDelete.Exists)
                            {
                                try
                                {
                                    ...
                                    System.Diagnostics.Debug.WriteLine("Deleted directory: " + this.LastShreddedPath);
                                }
                    ...
                    {
                        Console.WriteLine(
                            ">>> WARNING: Trying to wipe a directory with files inside. Nothing will be done.");
                    }
#endif
                }
                else
                {
                    System.Diagnostics.Debug.WriteLine(
                        ">>> WARNING: Trying to wipe a directory with subdirectories inside: it will be deleted later.");
                }
            }
        ...
        /// <param name="method">Shredding method</param>
        /// <param name="fileShreddingFeedback">
        ///     Callback entry point for feedback (can be null). The callback method will be called
        ///     every 1024 bytes shredded. An integer will be passed to callback method with the current kilobyte being erased. For
        ///     instance, if you want to erase a 200kb file, this delegate will call the method for 200 times passing to it
        ///     integers from 1 to 200.
        /// </param>
        /// <param name="phaseFeedback">
        ///     The phase feedback. A delegate is called at any shredding phase. IE: if the shredding
        ///     method overwrites 3 times with 0x00, 0xff, 0x00, the delegate is called 3 times with the current byte being written
        ///     as a message.
        /// </param>
            ...
            AutoResetEvent resetReq)
        {
            ...
            foreach (byte phaseByte in pattern)
            {
                ...
                try
                {
                    ...
                    chunk = GetChunkSequence(phaseByte);
