...
namespace LLCryptoLib.Hash
{
    ...
    public sealed class Hash : IHash, IDisposable
    {
        ...
        public string ComputeHashFromBytesStyle(byte[] contents, HexEnum style)
        {
            ...
            {
                this.Algorithm.ComputeHash(contents);
                byte[] result = this.Algorithm.Hash;
        ...
        public string ComputeHashFileStyleEx(string completeFileName, HexEnum style, CallbackEntry cbe,
            AutoResetEvent resetDemand)
        {
            ...
            try
            {
                ...
                if (this.Algorithm is HMACSHA1)
                {
                    ...
                    Debug.WriteLine("HMAC-SHA1 with password " + Hexer.BytesToHex(hmac.Key, DEFAULT_HEXSTYLE));
                }
#endif
        ...
        public string CompareHashClipboardEx(string filePath)
        {
            ...
            if (ClipboardUtils.CanPaste())
            {
                ...
                Debug.WriteLine(string.Format(Config.NUMBERFORMAT, "String in the clipboard is {0}", hashInMemory));

                int separatorIndex = hashInMemory.IndexOf("::");

                Debug.WriteLine(string.Format(Config.NUMBERFORMAT, "Index of :: is {0}", separatorIndex));
                ...
                if (separatorIndex != -1)
                {
                    ...
                    if (hashInMemory.Length == HashStringLen(DEFAULT_HEXSTYLE, this.Algorithm))
                    {
                        string hashFile = this.ComputeHashFile(filePath);
                        Debug.WriteLine(string.Format(Config.NUMBERFORMAT, "The hash for {0} is {1} ", filePath,
                            hashFile));

                        comparisonFeedback = FormatTabString(comparedFile, hashInMemory, filePath, hashFile);
                        comparisonFeedback += "\n\n";
        ...
        public bool CompareHashClipboard(string filePath)
        {
            ...
            Debug.WriteLine(string.Format(Config.NUMBERFORMAT, "CompareHashClipboard for file {0}", filePath));

            if (ClipboardUtils.CanPaste())
            {
                string hashInMemory = ClipboardUtils.Get();

                Debug.WriteLine(Config.NUMBERFORMAT,
                    string.Format(Config.NUMBERFORMAT, "String in the clipboard is {0}", hashInMemory));
        ...
        public bool CompareHash(string filePath, string hashToCompare)
        {
            ...
            Debug.WriteLine(string.Format(Config.NUMBERFORMAT, "CompareHash for file {0}", filePath));

            // Compute Hash of filePath
            string hashFile = this.ComputeHashFile(filePath);
            Debug.WriteLine(string.Format(Config.NUMBERFORMAT, "Hash of file is    > {0}", hashFile));
            Debug.WriteLine(string.Format(Config.NUMBERFORMAT, "Hash to compare is > {0}", hashToCompare));
        ...
        public string GetLibraryPath()
        {
            ...
            Debug.WriteLine("Library Path is: " + lPath);
#else
        ...
        private byte[] HashCode(string txt, Encoding textEncoding)
        {
            ...
            if (this.Algorithm != null)
            {
                ...
                this.Algorithm.ComputeHash(bytePhrase);
                result = this.Algorithm.Hash;
            }
