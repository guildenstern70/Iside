...
namespace LLCryptoLib.Hash
{
    ...
    public class Snefru2 : BlockHashAlgorithm
    {
        private uint[] accumulator;
        private readonly Snefru2Parameters parameters;
        ...
        public Snefru2(Snefru2Parameters param)
            : base(64 - param.Length/8)
        {
            lock (this)
            {
                if (param == null)
                {
                    throw new ArgumentNullException("param", "The Snefru2Parameters cannot be null.");
                }
        ...
        public override void Initialize()
        {
            lock (this)
            {
                this.accumulator = new uint[this.parameters.Length/32];
                base.Initialize();
            }
        }
        ...
        protected override void ProcessBlock(byte[] inputBuffer, int inputOffset)
        {
            lock (this)
            {
                ...
                Array.Copy(this.accumulator, 0, workBuffer, 0, this.accumulator.Length);
                Array.Copy(temp, 0, workBuffer, this.accumulator.Length, temp.Length);

                for (i = 0; i < this.parameters.Passes; i++)
                {
                    for (j = 0; j < 4; j++)
                    {
                        workBuffer[15] ^= SBoxes[2*i + 0, workBuffer[0] & 0xFF];
                        workBuffer[1] ^= SBoxes[2*i + 0, workBuffer[0] & 0xFF];
                        workBuffer[0] ^= SBoxes[2*i + 0, workBuffer[1] & 0xFF];
                        workBuffer[2] ^= SBoxes[2*i + 0, workBuffer[1] & 0xFF];
                        workBuffer[1] ^= SBoxes[2*i + 1, workBuffer[2] & 0xFF];
                        workBuffer[3] ^= SBoxes[2*i + 1, workBuffer[2] & 0xFF];
                        workBuffer[2] ^= SBoxes[2*i + 1, workBuffer[3] & 0xFF];
                        workBuffer[4] ^= SBoxes[2*i + 1, workBuffer[3] & 0xFF];
                        workBuffer[3] ^= SBoxes[2*i + 0, workBuffer[4] & 0xFF];
                        workBuffer[5] ^= SBoxes[2*i + 0, workBuffer[4] & 0xFF];
                        workBuffer[4] ^= SBoxes[2*i + 0, workBuffer[5] & 0xFF];
                        workBuffer[6] ^= SBoxes[2*i + 0, workBuffer[5] & 0xFF];
                        workBuffer[5] ^= SBoxes[2*i + 1, workBuffer[6] & 0xFF];
                        workBuffer[7] ^= SBoxes[2*i + 1, workBuffer[6] & 0xFF];
                        workBuffer[6] ^= SBoxes[2*i + 1, workBuffer[7] & 0xFF];
                        workBuffer[8] ^= SBoxes[2*i + 1, workBuffer[7] & 0xFF];
                        workBuffer[7] ^= SBoxes[2*i + 0, workBuffer[8] & 0xFF];
                        workBuffer[9] ^= SBoxes[2*i + 0, workBuffer[8] & 0xFF];
                        workBuffer[8] ^= SBoxes[2*i + 0, workBuffer[9] & 0xFF];
                        workBuffer[10] ^= SBoxes[2*i + 0, workBuffer[9] & 0xFF];
                        workBuffer[9] ^= SBoxes[2*i + 1, workBuffer[10] & 0xFF];
                        workBuffer[11] ^= SBoxes[2*i + 1, workBuffer[10] & 0xFF];
                        workBuffer[10] ^= SBoxes[2*i + 1, workBuffer[11] & 0xFF];
                        workBuffer[12] ^= SBoxes[2*i + 1, workBuffer[11] & 0xFF];
                        workBuffer[11] ^= SBoxes[2*i + 0, workBuffer[12] & 0xFF];
                        workBuffer[13] ^= SBoxes[2*i + 0, workBuffer[12] & 0xFF];
                        workBuffer[12] ^= SBoxes[2*i + 0, workBuffer[13] & 0xFF];
                        workBuffer[14] ^= SBoxes[2*i + 0, workBuffer[13] & 0xFF];
                        workBuffer[13] ^= SBoxes[2*i + 1, workBuffer[14] & 0xFF];
                        workBuffer[15] ^= SBoxes[2*i + 1, workBuffer[14] & 0xFF];
                        workBuffer[14] ^= SBoxes[2*i + 1, workBuffer[15] & 0xFF];
                        workBuffer[0] ^= SBoxes[2*i + 1, workBuffer[15] & 0xFF];

                        for (k = 0; k < workBuffer.Length; k++)
                        {
                            workBuffer[k] = Utilities.RotateRight(workBuffer[k], ShiftTable[j]);
                        }
                    }
                }

                this.accumulator[0] ^= workBuffer[15];
                this.accumulator[1] ^= workBuffer[14];
                this.accumulator[2] ^= workBuffer[13];
                this.accumulator[3] ^= workBuffer[12];
                if (this.parameters.Length == 256)
                {
                    this.accumulator[4] ^= workBuffer[11];
                    this.accumulator[5] ^= workBuffer[10];
                    this.accumulator[6] ^= workBuffer[9];
                    this.accumulator[7] ^= workBuffer[8];
                }
            }
        }
        ...
        protected override byte[] ProcessFinalBlock(byte[] inputBuffer, int inputOffset, int inputCount)
        {
            lock (this)
            {
                ...
                Array.Copy(Utilities.ULongToByte(size, EndianType.BigEndian), 0, temp, this.BlockSize - 8, 8);
                this.ProcessBlock(temp, 0);
        ...
        private static readonly int[] ShiftTable = new int[4] {16, 8, 16, 24};

        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Performance",
             "CA1814:PreferJaggedArraysOverMultidimensional", MessageId = "Member")] private static readonly uint[,]
            SBoxes = new uint[16, 256]
            {
                {
