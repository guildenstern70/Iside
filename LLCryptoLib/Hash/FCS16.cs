...
namespace LLCryptoLib.Hash
{
    ...
    public class FCS16 : System.Security.Cryptography.HashAlgorithm
    {
        private ushort hash;


        #region Table
        private static ushort[] LookupTable = new ushort[] {
        ...
        override protected void HashCore(byte[] array, int ibStart, int cbSize)
        {
            lock (this)
            {
                ...
                {
                    hash = (ushort)((hash >> 8) ^ LookupTable[(hash ^ (ushort)array[i]) & 0xFF]);
                }
            }
        }
        ...
        override protected byte[] HashFinal()
        {
            lock (this)
            {
                return Utilities.UShortToByte((ushort)(~hash), EndianType.BigEndian);
            }
        }
    }
}
