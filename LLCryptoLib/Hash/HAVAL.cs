...
namespace LLCryptoLib.Hash 
{
	...
	public class HAVAL : BlockHashAlgorithm 
    {
		...
		public HAVAL(HAVALParameters param) : base(128) 
        {
			lock (this) 
            {
				if (param == null) { throw new ArgumentNullException("param", "The HAVALParameters cannot be null."); }
		...
		override public void Initialize() 
        {
			lock (this) 
            {
				accumulator[0] = 0x243F6A88;
				accumulator[1] = 0x85A308D3;
				accumulator[2] = 0x13198A2E;
				accumulator[3] = 0x03707344;
				accumulator[4] = 0xA4093822;
				accumulator[5] = 0x299F31D0;
				accumulator[6] = 0x082EFA98;
				accumulator[7] = 0xEC4E6C89;
				base.Initialize();
			}
		}
		...
		override protected void ProcessBlock(byte[] inputBuffer, int inputOffset) 
        {
			lock (this) 
            {
				Transform(Utilities.ByteToUInt(inputBuffer, inputOffset, BlockSize));
			}
		}
		...
		override protected byte[] ProcessFinalBlock(byte[] inputBuffer, int inputOffset, int inputCount) 
        {
			lock (this) 
            {
				...
				FoldHash();
				return Utilities.UIntToByte(accumulator, 0, (parameters.Length / 32));
			}
		}
		...
		private void FoldHash() {
			...
			if (parameters.Length == 128) {
				...
				accumulator[0] += Utilities.RotateRight(temp, 19);

				temp = (uint)((accumulator[7] & (0x3F << 6)) | (accumulator[6] & 0x3F) | (accumulator[5] & (0x7F << 25)));
				accumulator[1] += Utilities.RotateRight(temp, 25);
				...
				accumulator[0] += Utilities.RotateRight(temp, 26);
