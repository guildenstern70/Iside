    ...
    public class RIPEMD320 : BlockHashAlgorithm
    {
        ...
        override public void Initialize()
        {
            lock (this)
            {
                ...
                accumulator[5] = 0x76543210;
                accumulator[6] = 0xFEDCBA98;
                accumulator[7] = 0x89ABCDEF;
                accumulator[8] = 0x01234567;
                accumulator[9] = 0x3C2D1E0F;
                base.Initialize();
            }
        }
        ...
        override protected void ProcessBlock(byte[] inputBuffer, int inputOffset)
        {
            lock (this)
            {
                ...
                for (i = 16; i < 32; i++)
                {
                    ...
                    A = E; E = D; D = Utilities.RotateLeft(C, 10); C = B;
                    B = Utilities.RotateLeft(T, s) + A;

                    s = Sp[i];
                    T = Ap + ((Bp & Dp) | (Cp & ~Dp)) + workBuffer[Rp[i]] + 0x5C4DD124;
                    Ap = Ep; Ep = Dp; Dp = Utilities.RotateLeft(Cp, 10); Cp = Bp;
                    Bp = Utilities.RotateLeft(T, s) + Ap;
                }
                T = D; D = Dp; Dp = T;
