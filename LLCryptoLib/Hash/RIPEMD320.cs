    ...
    public class RIPEMD320 : BlockHashAlgorithm
    {
        ...
        public override void Initialize()
        {
            lock (this)
            {
                ...
                this.accumulator[5] = 0x76543210;
                this.accumulator[6] = 0xFEDCBA98;
                this.accumulator[7] = 0x89ABCDEF;
                this.accumulator[8] = 0x01234567;
                this.accumulator[9] = 0x3C2D1E0F;
                base.Initialize();
            }
        }
        ...
        protected override void ProcessBlock(byte[] inputBuffer, int inputOffset)
        {
            lock (this)
            {
                ...
                B = this.accumulator[1];
                C = this.accumulator[2];
                D = this.accumulator[3];
                E = this.accumulator[4];
                Ap = this.accumulator[5];
                Bp = this.accumulator[6];
                Cp = this.accumulator[7];
                Dp = this.accumulator[8];
                Ep = this.accumulator[9];
                ...
                this.accumulator[5] += Ap;
                this.accumulator[6] += Bp;
                this.accumulator[7] += Cp;
                this.accumulator[8] += Dp;
                this.accumulator[9] += Ep;
            }
        }
