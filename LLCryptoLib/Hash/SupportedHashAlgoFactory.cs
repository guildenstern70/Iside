...
namespace LLCryptoLib.Hash
{
    ...
    public static class SupportedHashAlgoFactory
    {
        ...
        public static SupportedHashAlgo FromName(string xName)
        {
            ...
            if (xName.StartsWith("-"))
            {
                selectedAlgo = GetAlgo(AvailableHash.FAKE);
            }
            else
            {
        ...
        {
            return GetAlgo((AvailableHash) algoId);
        }
        ...
        public static SupportedHashAlgo GetAlgo(AvailableHash hashAlgo)
        {
            ...
            switch (hashAlgo)
            {
                ...
                case AvailableHash.ADLER32:
                    ...
                    sha.Description =
                        "Adler-32 is composed of two sums accumulated per byte: s1 is the sum of all bytes, s2 is the sum of all s1 values. Both sums are done modulo 65521. s1 is initialized to 1, s2 to zero.  The Adler-32 checksum is stored as s2*65536 + s1 in most-significant-byte order.";
                    break;

                case AvailableHash.CRC32:
                    sha = new SupportedHashAlgo(AvailableHash.CRC32, "CRC32", false, true, new CRC32());
                    sha.Description =
                        "CRC32 is not precisely a hash function: it is a cyclic redundancy checksum polynomial of 32-bit lengths of the input bytes.";
                    break;
                ...
                case AvailableHash.GHASH323:
                    sha = new SupportedHashAlgo(AvailableHash.GHASH323, "GHash-32-3", false, true,
                        new GHash(GHashParameters.GetParameters(GHashStandard.GHash_3)));
                    sha.Description = "An implementation of the GHash-32 hash function, with a shift of 3 bits";
                    break;

                case AvailableHash.GHASH325:
                    sha = new SupportedHashAlgo(AvailableHash.GHASH325, "GHash-32-5", false, true,
                        new GHash(GHashParameters.GetParameters(GHashStandard.GHash_5)));
                ...
                case AvailableHash.MD2:
                    ...
                    sha.Description =
                        "The MD2 algorithm is a 128 bit hash function invented by Ron Rivest. The original message is split into blocks of 512 bits. Each block is then mixed with each other for four rounds, after a series of XOR, AND, OR bit operations. Then the message is compressed and a final 128 bit string is extracted.";
                    break;

                case AvailableHash.MD4:
                    sha = new SupportedHashAlgo(AvailableHash.MD4, "MD4", false, true, new MD4());
                    sha.Description =
                        "The MD4 algorithm is a successor to MD2 algorithm. The original message is split into blocks of 512 bits. Each block is then mixed with each other for four rounds, after a series of XOR, AND, OR bit operations. Then the message is compressed and a final 128 bit string is extracted.";
                    break;

                case AvailableHash.MD5:
                    sha = new SupportedHashAlgo(AvailableHash.MD5, "MD5", false, true, new MD5CryptoServiceProvider());
                    sha.Description =
                        "The MD5 hash function invented by Ron Rivest. The original message is split into blocks of 512 bits. Each block is then mixed with each other for four rounds, after a series of XOR, AND, OR bit operations. Then the message is compressed and a final 128 bit string is extracted. MD5 is known to be one of the fastest hashing functions available for 32 bit CPU machines.";
                    break;

                case AvailableHash.SHA1:
                    sha = new SupportedHashAlgo(AvailableHash.SHA1, "SHA1", false, true, new SHA1Managed());
                    sha.Description =
                        "SHA1 is a cryptographic message digest algorithm similar to the MD5 family of hash functions developed by Rivest. It differs in that it adds an additional expansion operation, an extra round and the whole transformation was designed to accomodate the DSS block size for efficiency. The Secure Hash Algorithm takes a message of less than 264 bits in length and produces a 160-bit message digest which is designed so that it should be computationaly expensive to find a text which matches a given hash. ";
                    break;

                case AvailableHash.SHA224:
                    sha = new SupportedHashAlgo(AvailableHash.SHA224, "SHA224", false, true, new SHA224());
                    sha.Description =
                        "Similar to SHA256, SHA-224 message digest algorithm was proposed by NIST in the FIPS PUB 186-2 Change Notice 1.";
                    break;

                case AvailableHash.SHA256:
                    sha = new SupportedHashAlgo(AvailableHash.SHA256, "SHA256", false, true, new SHA256Managed());
                    sha.Description =
                        "Same as SHA1, this algorithm outputs a 256 bits string. It is among the best hash functions available today, used by American Encryption Standard (AES) algorithm.";
                    break;

                case AvailableHash.SHA384:
                    sha = new SupportedHashAlgo(AvailableHash.SHA384, "SHA384", false, false, new SHA384Managed());
                    sha.Description =
                        "Same as SHA1, this algorithm outputs a 384 bits string. It is a very complex algorithm, used by American Encryption Standard (AES) algorithm. This algorithm requires a relatively long time to be computed.";
                    break;

                case AvailableHash.SHA512:
                    sha = new SupportedHashAlgo(AvailableHash.SHA512, "SHA512", false, false, new SHA512Managed());
                    sha.Description =
                        "Same as SHA1, this algorithm outputs a 512 bits string. It is a very complex algorithm, used by American Encryption Standard (AES) algorithm. This algorithm requires a relatively long time to be computed.";
                    break;
                ...
                case AvailableHash.GOST:
                    ...
                    sha.Description =
                        "GOST R 34.11-94 is a 256-bit cryptographic hash function defined in the Russia's national standard GOST R 34.11-94 Information Technology - Cryptographic Information Security - Hash Function. The equivalent standard used by other member-states of the CIS is GOST 34.311-95.";
                    break;

                case AvailableHash.WHIRLPOOL:
                    sha = new SupportedHashAlgo(AvailableHash.WHIRLPOOL, "WHIRLPOOL", false, false, new Whirlpool());
                    sha.Description =
                        "WHIRLPOOL is a hash function designed by Vincent Rijmen and Paulo S. L. M. Barreto. uses Merkle-Damgård strengthening and the Miyaguchi-Preneel hashing scheme with a dedicated 512-bit block cipher.";
                    break;
                ...
                case AvailableHash.HAVAL128:
                    ...
                    sha.Description =
                        "HAVAL 128 bit is a one-way hashing algorithm designed in 1992. While many of its peers, including MD4 and MD5, have been fully or partially broken, no successful attack on HAVAL has been reported so far. Hence it can serve as a drop-in replacement of MD5. This implementation is the 128 bits HAVAL with 3 passes.";
                    break;
                ...
                case AvailableHash.HAVAL160:
                    ...
                    sha.Description =
                        "HAVAL 160 bit is a one-way hashing algorithm designed in 1992. While many of its peers, including MD4 and MD5, have been fully or partially broken, no successful attack on HAVAL has been reported so far. Hence it can serve as a drop-in replacement of MD5. This implementation is the 160 bits HAVAL with 3 passes.";
                    break;
                ...
                case AvailableHash.HAVAL192:
                    ...
                    sha.Description =
                        "HAVAL 192 bit is a one-way hashing algorithm designed in 1992. While many of its peers, including MD4 and MD5, have been fully or partially broken, no successful attack on HAVAL has been reported so far. Hence it can serve as a drop-in replacement of MD5. This implementation is the 192 bits HAVAL with 3 passes.";
                    break;
                ...
                case AvailableHash.HAVAL224:
                    ...
                    sha.Description =
                        "HAVAL 224 bit is a one-way hashing algorithm designed in 1992. While many of its peers, including MD4 and MD5, have been fully or partially broken, no successful attack on HAVAL has been reported so far. Hence it can serve as a drop-in replacement of MD5. This implementation is the 224 bits HAVAL with 3 passes.";
                    break;
                ...
                case AvailableHash.HAVAL256:
                    ...
                    sha.Description =
                        "HAVAL 256 bit is a one-way hashing algorithm designed in 1992. While many of its peers, including MD4 and MD5, have been fully or partially broken, no successful attack on HAVAL has been reported so far. Hence it can serve as a drop-in replacement of MD5. This implementation is the 256 bits HAVAL with 3 passes.";
                    break;
                ...
                case AvailableHash.HMACSHA1:
                    ...
                    sha.Description =
                        "HMAC can be used in combination with any iterated cryptographic hash function. MD5 and SHA-1 are examples. It uses a secret key for calculation and verification of the message authentication values.";
                    break;
                ...
                case AvailableHash.SKEIN256:
                    ...
                    sha.Description =
                        "Skein 256 bit is a cryptographic hash function and one out of five finalists in the NIST hash function competition to design what will become the SHA-3 standard, the intended successor of SHA-1 and SHA-2. According to Stefan Lucks, the name Skein refers to how the Skein function intertwines the input, similar to a coil of yarn, which is called a skein.";
                    break;
                ...
                case AvailableHash.SKEIN384:
                    ...
                    sha.Description =
                        "Skein 384 bit is a cryptographic hash function and one out of five finalists in the NIST hash function competition to design what will become the SHA-3 standard, the intended successor of SHA-1 and SHA-2. According to Stefan Lucks, the name Skein refers to how the Skein function intertwines the input, similar to a coil of yarn, which is called a skein.";
                    break;
                ...
                case AvailableHash.SKEIN512:
                    ...
                    sha.Description =
                        "Skein 512 bit is a cryptographic hash function and one out of five finalists in the NIST hash function competition to design what will become the SHA-3 standard, the intended successor of SHA-1 and SHA-2. According to Stefan Lucks, the name Skein refers to how the Skein function intertwines the input, similar to a coil of yarn, which is called a skein.";
                    break;
