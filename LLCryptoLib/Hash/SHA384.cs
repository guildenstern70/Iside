...
namespace LLCryptoLib.Hash
{
    ...
    public class SHA384 : BlockHashAlgorithm
    {
        ...
        public override void Initialize()
        {
            lock (this)
            {
                this.accumulator[0] = 0xCBBB9D5DC1059ED8;
                this.accumulator[1] = 0x629A292A367CD507;
                this.accumulator[2] = 0x9159015A3070DD17;
                this.accumulator[3] = 0x152FECD8F70E5939;
                this.accumulator[4] = 0x67332667FFC00B31;
                this.accumulator[5] = 0x8EB44A8768581511;
                this.accumulator[6] = 0xDB0C2E0D64F98FA7;
                this.accumulator[7] = 0x47B5481DBEFA4FA4;
                base.Initialize();
            }
        }
        ...
        protected override void ProcessBlock(byte[] inputBuffer, int inputOffset)
        {
            lock (this)
            {
                ...
                ulong a = this.accumulator[0];
                ulong b = this.accumulator[1];
                ulong c = this.accumulator[2];
                ulong d = this.accumulator[3];
                ulong e = this.accumulator[4];
                ulong f = this.accumulator[5];
                ulong g = this.accumulator[6];
                ulong h = this.accumulator[7];
                ulong T1, T2;


                ulong[] temp = Utilities.ByteToULong(inputBuffer, inputOffset, this.BlockSize, EndianType.BigEndian);
        ...
        protected override byte[] ProcessFinalBlock(byte[] inputBuffer, int inputOffset, int inputCount)
        {
            lock (this)
            {
                ...
                paddingSize = this.BlockSize - 16 - paddingSize;
                if (paddingSize < 1)
                {
                ...
                return Utilities.ULongToByte(this.accumulator, 0, this.accumulator.Length - 2, EndianType.BigEndian);
            }
        }
        ...
        {
            return Utilities.RotateRight(x, 1) ^ Utilities.RotateRight(x, 8) ^ this.R(7, x);
        }

        private ulong Sig1R(ulong x)
        {
            return Utilities.RotateRight(x, 19) ^ Utilities.RotateRight(x, 61) ^ this.R(6, x);
        }
