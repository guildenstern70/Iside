// $Id: Panama.cs,v 1.1 2006/09/13 09:21:15 asaltar Exp $
	...
	public class Panama : BlockHashAlgorithm {
		...
		override protected void ProcessBlock(byte[] inputBuffer, int inputOffset) {
			lock (this) {
				...
				// Pi
				pi[ 0] = gamma[0];
				pi[ 1] = Utilities.RotateLeft(gamma[ 7],  1);
				pi[ 2] = Utilities.RotateLeft(gamma[14],  3);
				pi[ 3] = Utilities.RotateLeft(gamma[ 4],  6);
				pi[ 4] = Utilities.RotateLeft(gamma[11], 10);
				pi[ 5] = Utilities.RotateLeft(gamma[ 1], 15);
				pi[ 6] = Utilities.RotateLeft(gamma[ 8], 21);
				pi[ 7] = Utilities.RotateLeft(gamma[15], 28);
				pi[ 8] = Utilities.RotateLeft(gamma[ 5],  4);
				pi[ 9] = Utilities.RotateLeft(gamma[12], 13);
				pi[10] = Utilities.RotateLeft(gamma[ 2], 23);
				pi[11] = Utilities.RotateLeft(gamma[ 9],  2);
				pi[12] = Utilities.RotateLeft(gamma[16], 14);
				pi[13] = Utilities.RotateLeft(gamma[ 6], 27);
				pi[14] = Utilities.RotateLeft(gamma[13],  9);
				pi[15] = Utilities.RotateLeft(gamma[ 3], 24);
				pi[16] = Utilities.RotateLeft(gamma[10],  8);
		...
		private byte[] Pull() {
			...
			Array.Copy(state, 0, accumulator, 0, 17);
			return Utilities.UIntToByte(hash, EndianType.LittleEndian);
		}
	}
}
