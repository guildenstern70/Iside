...
namespace LLCryptoLib.Hash
{
    ...
    public class FCS32 : System.Security.Cryptography.HashAlgorithm
    {
        private uint hash;


        #region Table
        private static uint[] LookupTable = new uint[] {
        ...
        override protected void HashCore(byte[] array, int ibStart, int cbSize)
        {
            lock (this)
            {
                ...
                {
                    hash = (uint)((hash >> 8) ^ LookupTable[(hash ^ (uint)array[i]) & 0xFF]);
                }
            }
        }
        ...
        override protected byte[] HashFinal()
        {
            lock (this)
            {
                return Utilities.UIntToByte((uint)(~hash), EndianType.BigEndian);
            }
        }
    }
}
