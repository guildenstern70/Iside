...
namespace LLCryptoLib.Hash
{
    ...
    public class Cksum : HashAlgorithm
    {
        #region Table
        ...
        protected override void HashCore(byte[] array, int ibStart, int cbSize)
        {
            lock (this)
            {
                ...
                {
                    this.checksum = (this.checksum << 8) ^ LookupTable[((this.checksum >> 24) ^ array[i]) & 0xFF];
                    this.count++;
                }
            }
        }
        ...
        protected override byte[] HashFinal()
        {
            lock (this)
            {
                ulong tempCount = this.count;
                for (; tempCount != 0; tempCount >>= 8)
                {
                    this.checksum = (this.checksum << 8) ^
                                    LookupTable[((this.checksum >> 24) ^ (int) (tempCount & 0xFF)) & 0xFF];
                }

                return Utilities.UIntToByte(~this.checksum & 0xFFFFFFFF, EndianType.BigEndian);
            }
        }
    }
}