...
namespace LLCryptoLib.Hash
{
    ...
    public class Cksum : System.Security.Cryptography.HashAlgorithm
    {
        ...
        #region Table
        static private uint[] LookupTable = new uint[] {
        ...
        override protected void HashCore(byte[] array, int ibStart, int cbSize)
        {
            lock (this)
            {
                ...
                {
                    checksum = (checksum << 8) ^ LookupTable[((checksum >> 24) ^ array[i]) & 0xFF];
                    count++;
                }
            }
        }
        ...
        override protected byte[] HashFinal()
        {
            lock (this)
            {
                ulong tempCount = count;
                for (; tempCount != 0; tempCount >>= 8)
                {
                    checksum = (checksum << 8) ^ LookupTable[((checksum >> 24) ^ (int)(tempCount & 0xFF)) & 0xFF];
                }
