...
namespace LLCryptoLib.Crypto
{
    ...
    public class Cast5Managed : SymmetricAlgorithm, ICryptoTransform
    {
        ...
        public Cast5Managed()
        {
            KeySizeValue = Cast5Managed.MAX_KEY_LENGTH << 3;

            LegalBlockSizesValue = new KeySizes[1];
            LegalBlockSizesValue[0] = new KeySizes(BLOCK_SIZE * 8, BLOCK_SIZE * 8, BLOCK_SIZE);

            LegalKeySizesValue = new KeySizes[1];
            LegalKeySizesValue[0] = new KeySizes(0, Cast5Managed.MAX_KEY_LENGTH << 3, 8);
        }
        ...
        public override byte[] IV
        {
            ...
            set
            {
                ...
                if (value.Length != Cast5Managed.BLOCK_SIZE)
                {
                    throw new CryptographicException("Invalid IV length: get " + value.Length + ", expected " + Cast5Managed.BLOCK_SIZE + " bytes.");
                }
        ...
        {
            get { return Cast5Managed.BLOCK_SIZE; }
        }
        ...
        public int TransformBlock(byte[] inputBuffer, int inputOffset, int inputCount, byte[] outputBuffer, int outputOffset)
        {
            ...
            if (0 != inputCount % Cast5Managed.BLOCK_SIZE)
            {
            ...
            {
                const int blockSize = Cast5Managed.BLOCK_SIZE;
                int offset = 0;
        ...
        {
            if (inputCount > Cast5Managed.BLOCK_SIZE)
            {
                throw new CryptographicException("Input count greater than block size");
            }

            byte[] outputBuffer = new byte[Cast5Managed.BLOCK_SIZE];
            byte[] buffer = new byte[Cast5Managed.BLOCK_SIZE];  // Initializes buffer with zeroes
            ...
            if (this.isEncryption)
            {
                int outputBufferLen = Cast5Managed.BLOCK_SIZE +
                                      (Cast5Managed.BLOCK_SIZE == inputCount ? Cast5Managed.BLOCK_SIZE : 0);
                ...
                if (this.Padding == PaddingMode.PKCS7)
                {
                    ...
                    byte bytesToFill = (byte)(Cast5Managed.BLOCK_SIZE - inputCount);
                ...
                if (this.isCBC)
                {
                    Cast5.EncryptCBC(this.scheduledKey, buffer, inputOffset, Cast5Managed.BLOCK_SIZE, this.GetIV());
                }
                else
                {
                ...
                Array.Copy(buffer, outputBuffer, Cast5Managed.BLOCK_SIZE);
            }
            else  // Decryption
            {
                if (0 == inputCount)
                {
                    ...
                    // to the output block
                    Array.Copy(this.lastBlock, buffer, Cast5Managed.BLOCK_SIZE);
                ...
                else
                {
                    if (this.isCBC)
                    {
                        Cast5.DecryptCBC(this.scheduledKey, buffer, inputOffset, Cast5Managed.BLOCK_SIZE, this.GetIV());
                    }
                    else
                    {
                ...
                if (inputCount > 0)
                {
                    ...
                    Array.Copy(buffer, 0, outputBuffer, Cast5Managed.BLOCK_SIZE, rest);
                }
                else
                {
        ...
        /// </summary>
        /// <returns>The number of byte to return after having de-padded the last block
        ...
        private int CheckLastBlockPadding(byte[] buffer)
        {
            ...
            if (PaddingMode.PKCS7 == PaddingValue)
            {
                ...
                if (Cast5Managed.BLOCK_SIZE < (int)bytesToFill)
                {
